...
function readCampaignList() {
  ...
  return firebase.database().ref('/users/' + userId + '/campaigns').once('value').then(function(snapshot) {
    ...
    for(var key in snapshot.val()) {
      ...
      div.innerHTML = '<a href="/editor?id='+key+'"><span><i class="icon-news"></i> '+keyData.title+'</span></a><i class="icon-x delete" data-key="'+key+'" alt="delete campaign" title="delete campaign"></i>'
